...
$PAGE->set_url('/question/type/essay/comment.php', ['attempt' => $attemptid, 'slot' => $slot]);
...
$PAGE->set_title(get_string('manualgradequestion', 'quiz', [
    ...
    'quiz' => format_string($attemptobj->get_quiz_name()),
...
if (data_submitted()) {
    if (question_engine::is_manual_grade_in_range($attemptobj->get_uniqueid(), $slot)) {
...
echo '<form method="post" class="mform" id="manualgradingform" action="' . $CFG->wwwroot . '/question/type/essay/comment.php?attempt='.$attemptobj->get_attemptid().'">';
